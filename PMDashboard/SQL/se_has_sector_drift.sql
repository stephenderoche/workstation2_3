...
if exists (select * from sysobjects where name = 'se_has_sector_drift')
begin
	drop procedure se_has_sector_drift
	print 'PROCEDURE: se_has_sector_drift dropped'
end
go


create procedure [dbo].[se_has_sector_drift]
...
select @has_drift =
     case 
	     when   ABS(((Coalesce(#holdings_percent.Holdings_percent,0) - Coalesce(#benchmark_model.benchmark_percent,0))/100)) < abs(((Coalesce(#benchmark_model.benchmark_percent,0)/100)* .1))then 'N'
...
--		select @has_drift = 'N' --as has_sector_drift
		...
		--se_has_sector_drift 20419,'D.Eq GICS',198
